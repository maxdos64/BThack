		...
		case HCI_EVENT_PACKET:
			switch (hci_event_packet_get_type(packet)) {
				...
				case SM_EVENT_NUMERIC_COMPARISON_REQUEST:
					...
					printf("RESP: using passkey %d\n", passkey);
					ipc_write((char*) &passkey, sizeof(uint32_t));
					/* wait for passkey finish */
					ipc_read(buf, 1);
					printf("RESP: passkey finished\n");
					sm_numeric_comparison_confirm(sm_event_passkey_display_number_get_handle(packet));
					printf("RESP: confirmed numeric comparison\n");
					break;
				case SM_EVENT_PASSKEY_DISPLAY_NUMBER:
...
int main(int argc, const char * argv[])
{
	...
	if(cpid == 0) /* Initiator (child) performing Passkey Entry with dongle*/
	{
	...
	else /* Responder (parent) performing Numeric Comparison with mobile phone*/
	{
		...
		// wait for initiator to find target responder
		//char buf[1];
		//ipc_read(buf, 1);
		hci_power_control(HCI_POWER_ON);

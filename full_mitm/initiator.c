...
int main(int argc, const char * argv[])
{
	...
	if (!strcmp(argv[3], "NO_INPUT_NO_OUTPUT"))
	{
		printf("NOTE: THIS IS NOT VALID FOR METHOD CONFUSION ATTACK!\n");
		sm_set_authentication_requirements(SM_AUTHREQ_SECURE_CONNECTION);
		sm_set_io_capabilities(IO_CAPABILITY_NO_INPUT_NO_OUTPUT);
	}
	else if (!strcmp(argv[3], "DISPLAY_ONLY"))
	{
		printf("NOTE: THIS IS NOT VALID FOR METHOD CONFUSION ATTACK!\n");
		sm_set_io_capabilities(IO_CAPABILITY_DISPLAY_ONLY);
		sm_set_authentication_requirements(SM_AUTHREQ_SECURE_CONNECTION|SM_AUTHREQ_MITM_PROTECTION);
	}
	else if (!strcmp(argv[3], "KEYBOARD_ONLY"))
	{
		sm_set_io_capabilities(IO_CAPABILITY_KEYBOARD_ONLY);
		sm_set_authentication_requirements(SM_AUTHREQ_SECURE_CONNECTION|SM_AUTHREQ_MITM_PROTECTION);
	}
	else if (!strcmp(argv[3], "KEYBOARD_DISPLAY"))
	{
		sm_set_io_capabilities(IO_CAPABILITY_KEYBOARD_DISPLAY);
		sm_set_authentication_requirements(SM_AUTHREQ_SECURE_CONNECTION|SM_AUTHREQ_MITM_PROTECTION);
	}
	else if (!strcmp(argv[3], "DISPLAY_YES_NO"))
	{
